...
void main() {
  ...
  testWidgets("affiche un bouton d'accès au règlement", (tester) async {
    await pumpHomeScreen(tester);

    expect(find.byKey(const Key('rules_fab')), findsOneWidget);

    await disposeWidgetTree(tester);
  });

  testWidgets('taper sur le bouton règlement ouvre RulesScreen', (
    tester,
  ) async {
    await pumpHomeScreen(tester);

    await tester.tap(find.byKey(const Key('rules_fab')));
